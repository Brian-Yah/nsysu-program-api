...
`structured_requirements.course_count_constraints` 保存明文指定的採計門數上限。每筆包含穩定 `constraint_id`、`kind: max_courses`、`course_names`、`max_courses`、課程群組、來源頁、原文及驗證狀態。課名可能沒有課號，consumer 應使用相同 academic version 的 catalog snapshot 配對；不得把同一 constraint 中超過上限的課程學分全部累加。
